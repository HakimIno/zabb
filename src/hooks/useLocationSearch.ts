...
import { SearchRequest, LocationResult } from '@/src/services/search/searchService';
...
export const useLocationSearch = ({ currentLocation }: UseLocationSearchProps) => {
    ...
    // Helper functions
    const updateFieldText = (fieldType: FieldType, text: string) => {
    ...
    const getFieldState = (fieldType: FieldType) => searchFields[fieldType];
    ...
    }, [focusedField, searchFields.pickup.text, searchFields.destination.text]);
