...
export function MapMarker({
  ...
  return (
    <Mapbox.PointAnnotation id={location.id} coordinate={location.coordinates} onSelected={onPress}>
...
export function CustomMarker({ coordinate, title, onPress, color = '#EF4444' }: CustomMarkerProps) {
  return (
    <Mapbox.PointAnnotation
      ...
      <View className="items-center" style={{ minWidth: 80 }}>
        ...
        {title && (
          <View className="mt-1 bg-white dark:bg-gray-800 px-2 py-1 rounded shadow">
            <Text className="text-xs font-medium text-gray-900 dark:text-gray-100">{title}</Text>
          </View>
        )}
      </View>
    </Mapbox.PointAnnotation>
