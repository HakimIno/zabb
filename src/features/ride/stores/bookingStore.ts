...
import { directionsService } from '@/features/map/services/directions';
import type { LocationResult } from '@/features/map/services/geocoding';
import { MOCK_DRIVER, PAYMENT_METHODS, RIDE_TYPES } from '@/features/ride/components/constants';
import type {
  BookingStep,
  Driver,
  PaymentMethod,
  RideType,
} from '@/features/ride/components/types';
    ...
    handleMapPress: async (coordinate) => {
      ...
      try {
        if (isSelectingDestination) {
          const { geocodingService } = await import('@/features/map/services/geocoding');
...
export const useBookingSelectors = {
  ...
  isSelectingDestination: () =>
    useBookingStore((state) => state.bottomSheetState.isSelectingDestination),
  ...
  canConfirmBooking: () =>
    useBookingStore(
      (state) => state.destination && state.pickupLocation && state.routeInfo.coordinates.length > 0
    ),
};
